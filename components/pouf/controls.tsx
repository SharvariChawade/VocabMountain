...
export function Tooltip({ tip, children }: { tip: string; children: ReactNode }) {
  return (
    <RTooltip.Root>
      <RTooltip.Trigger asChild><span className="pouf-tip-anchor">{children}</span></RTooltip.Trigger>
      <RTooltip.Portal>
        ...
        <RDialog.Content className={size === 'lg' ? 'pouf-dialog pouf-dialog--lg' : 'pouf-dialog'}>
          <Stack gap={4}>
            <div className="pouf-dialog__head">
              <Stack gap={1}>
                <RDialog.Title asChild><div><Heading level={3}>{title}</Heading></div></RDialog.Title>
                {description && (
              ...
              <RDialog.Close asChild><Button variant="quiet" size="sm" label="Close"><Icon name="close" size="sm" /></Button></RDialog.Close>
            </div>
    ...
    <RAlert.Root>
      ...
      <RAlert.Portal>
        ...
        <RAlert.Content className="pouf-dialog">
          <Stack gap={4}>
            <RAlert.Title asChild><div><Heading level={3}>{title}</Heading></div></RAlert.Title>
            <RAlert.Description asChild><div><Text muted>{body}</Text></div></RAlert.Description>
            {details}
            <Row gap={3} justify="end">
              <RAlert.Cancel asChild><Button variant="quiet" size="sm">{cancelLabel}</Button></RAlert.Cancel>
              <RAlert.Action asChild><Button tone={tone} size="sm" onClick={onConfirm} loading={loading}>{confirmLabel}</Button></RAlert.Action>
            </Row>
          </Stack>
        </RAlert.Content>
      </RAlert.Portal>
